...
constexpr uint32_t kStage9VectorChunkElements = 4 * kNzTileSize;
constexpr unsigned kStage9MaximumLogicalFlagId = 15;
...
static bool verifyPlan(const PostCVSplitSchedulePlan &plan) {
  ...
  for (const PostCVSplitVectorLanePlan &lane : plan.vectorLanes) {
    ...
    if (lane.lane >= lanes || lane.rows != plan.recurrence.vectorRows ||
        lane.chunkWidth != kStage9VectorChunkElements ||
...
PostCVSplitSchedulePlan buildPostCVSplitSchedulePlan(
    const CVSplitCostModelRequestSet &requests,
  ...
  plan.maximumLogicalFlagId = kStage9MaximumLogicalFlagId;
  ...
  for (unsigned lane = 0; lane < lanes; ++lane) {
    ...
    plan.vectorLanes.push_back(
        {lane, scoreTransfer.rows, kStage9VectorChunkElements,
         scoreTransfer.columns / kStage9VectorChunkElements, true});
  }
...
void logPostCVSplitSchedulePlan(const PostCVSplitSchedulePlan &plan) {
  LLVM_DEBUG({
    llvm::dbgs() << "[cv-split] stage9-plan status="
    ...
    llvm::dbgs() << "[cv-split] stage9-geometry rows="
                 ...
                 << (!plan.vectorLanes.empty() &&
                         ...
                         : "no")
                 << "\n";
    llvm::dbgs() << "[cv-split] stage9-memory score-depth="
                 << plan.scoreLiveDepth
    ...
    llvm::dbgs() << "[cv-split] stage9-flags first="
    ...
    llvm::dbgs() << "[cv-split] stage9-backend auto-bind=off graph-sync=off"
                    " vf-merge="
                 << plan.backend.vfMergeLevel << " attribute-emitted=no\n";
    for (const PostCVSplitSlotAssignment &slot : plan.slots)
      llvm::dbgs() << "[cv-split] stage9-slot role=" << roleName(slot.role)
                   << " lane=" << slot.lane << " slot=" << slot.slot
                   << "\n";
    for (const PostCVSplitEventPlan &event : plan.events)
      llvm::dbgs() << "[cv-split] stage9-event flag="
                   << event.logicalFlagId
    ...
    for (const PostCVSplitReductionStep &step : plan.reductionSteps)
      llvm::dbgs() << "[cv-split] stage9-reduction level=" << step.level
                   << " left=" << step.leftValue
                   << " right=" << step.rightValue
                   << " result=" << step.resultValue << "\n";
    llvm::dbgs() << "[cv-split] stage9-recurrence maximum="
