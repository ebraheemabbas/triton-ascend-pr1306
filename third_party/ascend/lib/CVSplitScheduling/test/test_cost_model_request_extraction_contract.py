#!/usr/bin/env python3
"""Source-contract checks for Cost-model request extraction."""
...
def test_cost_request_option_is_default_off_and_propagated() -> None:
...
def test_cost_requests_own_array_backing_and_exact_types() -> None:
...
def test_cost_request_extraction_is_read_only_and_kernel_agnostic() -> None:
    header = read(INCLUDE / "CostModelRequestExtraction.h")
    source = read(LIB / "CostModelRequestExtraction.cpp")
...
def test_cost_request_extraction_precedes_transfer_emission() -> None:
    cpp = read(LIB / "CVSplitScheduling.cpp")
...
def test_cost_request_build_and_diagnostics_are_owned() -> None:
...
if __name__ == "__main__":
    test_cost_request_option_is_default_off_and_propagated()
    test_cost_requests_own_array_backing_and_exact_types()
    test_cost_request_extraction_is_read_only_and_kernel_agnostic()
    test_cost_request_extraction_precedes_transfer_emission()
    test_cost_request_build_and_diagnostics_are_owned()
    print("Cost-model request extraction source contract: PASS")
