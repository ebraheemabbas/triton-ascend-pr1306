#!/usr/bin/env python3
"""Source and reference-policy contracts for Stage 9.3."""
...
def test_option_is_default_off_and_fully_propagated() -> None:
    ...
    pybind = read(PYBIND)
    assert 'Option<"enableStage9DetachedScheduleDiagnostics"' in passes
    option = passes.split(
        'Option<"enableStage9DetachedScheduleDiagnostics"', 1)[1]
    assert '"bool", /*default*/"false"' in option.split('>,', 1)[0]
    assert "options.enableStage9DetachedScheduleDiagnostics" in cpp
    assert "opts.enableStage9DetachedScheduleDiagnostics" in pybind
    assert "cv_split_enable_stage9_detached_schedule_diagnostics: bool = False" in backend
    assert '"cv_split_enable_stage9_detached_schedule_diagnostics"' in backend
...
def test_integration_precedes_live_transfer_mutation() -> None:
    cpp = read(LIB / "CVSplitScheduling.cpp")
    stage92 = cpp.index("buildPostCVSplitSchedulePlan(")
    stage93 = cpp.index("buildPostCVSplitDetachedSchedule(")
    transfer = cpp.index("insertCrossScopeTransfers(", stage93)
    assert stage92 < stage93 < transfer
...
if __name__ == "__main__":
    ...
    print("Stage 9.3 detached atomic schedule source contract: PASS")
