#!/usr/bin/env python3
"""Source contracts for Stage 9.4a semantic anchor binding."""
...
def test_option_is_default_off_and_propagated() -> None:
    ...
    pybind = read(PYBIND)
    assert 'Option<"enableStage94AnchorBindingDiagnostics"' in passes
    option = passes.split(
        'Option<"enableStage94AnchorBindingDiagnostics"', 1)[1]
    assert '"bool", /*default*/"false"' in option.split('>,', 1)[0]
    assert "options.enableStage94AnchorBindingDiagnostics" in cpp
    assert "opts.enableStage94AnchorBindingDiagnostics" in pybind
    assert "cv_split_enable_stage94_anchor_binding_diagnostics: bool = False" in backend
    assert '"cv_split_enable_stage94_anchor_binding_diagnostics"' in backend
...
def test_binding_runs_after_detached_plan_before_transfer_mutation() -> None:
    ...
    assert "stage94-binding unavailable" in cpp
...
if __name__ == "__main__":
    ...
    print("Stage 9.4a semantic anchor binding source contract: PASS")
