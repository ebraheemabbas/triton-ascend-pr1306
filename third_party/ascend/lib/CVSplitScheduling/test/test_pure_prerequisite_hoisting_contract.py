...
def test_prerequisite_hoist_options_are_default_off_and_propagated():
...
def test_prerequisite_hoist_uses_transfer_and_ssa_facts_only():
...
def test_prerequisite_hoist_rejects_unsafe_operands():
    cpp = read(LIB / "PurePrerequisiteHoisting.cpp")
...
def test_prerequisite_hoist_runs_between_transfer_and_scope_materialization():
    cpp = read(LIB / "CVSplitScheduling.cpp")
...
def test_prerequisite_hoist_build_owns_implementation():
    cmake = read(LIB / "CMakeLists.txt")
    assert "PurePrerequisiteHoisting.cpp" in cmake


if __name__ == "__main__":
    test_prerequisite_hoist_options_are_default_off_and_propagated()
    test_prerequisite_hoist_uses_transfer_and_ssa_facts_only()
    test_prerequisite_hoist_rejects_unsafe_operands()
    test_prerequisite_hoist_runs_between_transfer_and_scope_materialization()
    test_prerequisite_hoist_build_owns_implementation()
    print("Pure-prerequisite hoisting source contract: PASS")
